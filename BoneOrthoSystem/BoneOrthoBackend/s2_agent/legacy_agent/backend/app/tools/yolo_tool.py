...
PROJECT_ROOT = Path(__file__).resolve().parents[6]   # .../BoneOrthoBackend
DEFAULT_MODEL = PROJECT_ROOT / "s1_detection" / "model" / "best.pt"
# 上傳資料夾（要跟 main.py 的 UPLOAD_DIR 一樣）
UPLOAD_DIR = PROJECT_ROOT / "data" / "uploads"
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)

# 模型
MODEL_PATH = Path(os.getenv("S1_YOLO_MODEL_PATH", str(DEFAULT_MODEL))).resolve()
...
def get_bone_label_for_display(yolo_name: str) -> str:
    ...
    如果資料庫查不到，就直接回傳原本英文。
    """
    row = get_bone_zh_en_by_en(yolo_name)
    if row:
        zh, en_db = row
    else:
        zh, en_db = None, None
