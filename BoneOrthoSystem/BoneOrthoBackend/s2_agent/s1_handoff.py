...
def _build_seed_text(case_id: int, dets: List[Dict[str, Any]], user_question: str) -> str:
    ...
    "2) 目前提供的是 AI 偵測摘要，不代表完整醫學診斷；若提到骨折、脫臼、病變或關節異常，請用『常見需留意重點』方式說明，不可直接判定此影像一定有異常。\n"
    "3) 回答時請優先說明：偵測到哪些骨頭、它們的位置與功能、X 光判讀常見觀察點、注意事項，以及 2～3 個延伸學習問題。\n"
    "4) 請使用衛教與教學語氣，避免做出確診、治療指示或取代醫師判斷。\n"
)
