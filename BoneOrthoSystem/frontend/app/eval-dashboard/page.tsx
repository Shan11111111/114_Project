...
function logBlob(log: EvalLog) {
    return `${log.question} ${log.rag_mode} ${log.created_at} ${log.eval_json || ''
        }`.toLowerCase();
}
                ...
                <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className={`rounded-xl border px-4 py-2 text-sm ${autoRefresh
                ...
                <div className="mb-6 flex flex-wrap gap-2">
                    {QUICK_FILTERS.map((filter) => (
                        <button
                            ...
                            className={`rounded-full border px-4 py-2 text-sm transition ${quickFilter === filter.key
                                    ...
                                    {loading ? (
                                        ...
                                        pagedLogs.map((log) => (
                                            <tr
                                                ...
                                                className={`cursor-pointer border-t hover:bg-slate-50 ${selectedLog?.eval_id === log.eval_id ? 'bg-slate-50' : ''
                                                    }`}
                                            >
                                ...
                                <div className="max-h-[420px] space-y-3 overflow-y-auto pr-1">
                                    {gaps.map((gap) => (
                                        <button
                                            ...
                                            className={`w-full rounded-xl border p-4 text-left hover:bg-orange-50 ${selectedGap?.gap_id === gap.gap_id
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        ...
                                                        window.location.href =
                                                            `/llm/materials?keyword=${encodeURIComponent(
                                                                gap.suggested_query
                                                            )}`;
                                                    }}
                                                    className="rounded-lg bg-orange-500 px-2 py-1 text-xs text-white"
                                                >
                                                    補充教材
                                                </button>
                                        ...
                                        {!selectedDetails.length ? (
                                            ...
                                            <div className="max-h-[720px] space-y-3 overflow-y-auto pr-1">
                                                {selectedDetails.map((d: any, idx: number) => (
                                                    <div
                                                        key={idx}
                                                        className={`rounded-xl border p-4 ${d.supported
                                                    ...
                                                    >
                                                        <div className="mb-2 flex items-center justify-between">
                                                            ...
                                                            <span
                                                                className={`rounded-full px-2 py-1 text-xs font-semibold ${d.supported
